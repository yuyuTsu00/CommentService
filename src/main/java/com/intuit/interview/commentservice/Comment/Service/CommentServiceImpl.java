...
//    })
  ...
  //  @Cacheable(value = "paginatedPostComment", key = "#postId")
    public PaginatedResponse<CommentThreadDto> commentThreadsForPost(String postId, Pageable pageable)
    {
        ...
        return PaginatedResponse.<CommentThreadDto>builder().items(responseList).start(pageable.getPageNumber() + 1).count(list.size()).build();
    }

 //   @Cacheable(value = "paginatedComment", key = "#threadId")
    public PaginatedResponse<CommentThreadDto> commentThreadsForComment(String threadId, Pageable pageable)
    {
    ...
    private List<CommentThreadDto> dbObjectMapCommentThreadObject(List<BasicDBObject> list)
    {
        ...
        for(BasicDBObject object : list)
        {
            ...
            commentThreadDto.setDislikeCounter(Integer.parseInt(docs.get(0).get("dislikeCounter").toString()));
